...
public class PeerImpl extends AbstractPeer implements IPeer {
	...
	private static final Logger logger = LoggerFactory
			.getLogger(PeerImpl.class);
	protected static final DiameterMessageValidator _VALDIATOR_ = DiameterMessageValidator
			.getInstance();
	// Properties
	...
	// Feature
	protected boolean useUriAsFQDN = false; // Use URI as orign host name into
	// CER command
	...
	protected IConnectionListener connListener = new IConnectionListener() {
		...
		public void messageReceived(String connKey, IMessage message) {
			...
			try {
				...
				if (logger.isDebugEnabled()) {
					...
					logger.debug("Receive message type {} to peer {}",
							new Object[] { type, connKey });
				}
				switch (type) {
				case CAPABILITIES_EXCHANGE_REQUEST:
					fsm.handleEvent(new FsmEvent(req ? CER_EVENT : CEA_EVENT,
							message, connKey));
					break;
				case DEVICE_WATCHDOG_REQUEST:
					fsm.handleEvent(new FsmEvent(req ? DWR_EVENT : DWA_EVENT,
							message, connKey));
					break;
				case DISCONNECT_PEER_REQUEST:
					fsm.handleEvent(new FsmEvent(req ? DPR_EVENT : DPA_EVENT,
							message));
					break;
				default:
				...
				if (req) {
					try {
						...
						message.getAvps().addAvp(Avp.RESULT_CODE,
								ResultCode.TOO_BUSY, true);
		...
		public void internalError(String connKey, IMessage message,
	...
	public PeerImpl(PeerTableImpl table, int rating, URI remotePeer, String ip,
			String portRange, IMetaData metaData, Configuration config,
		...
		this(table, rating, remotePeer, ip, portRange, metaData, config,
				peerConfig, fsmFactory, trFactory, parser, statisticFactory,
				concurrentFactory, null);
	}

	protected PeerImpl(final PeerTableImpl table, int rating, URI remotePeer,
			String ip, String portRange, IMetaData metaData,
			Configuration config, Configuration peerConfig,
			IFsmFactory fsmFactory, ITransportLayerFactory trFactory,
			IMessageParser parser, IStatisticFactory statisticFactory,
			IConcurrentFactory concurrentFactory, IConnection connection)
			throws InternalException, TransportException {
		...
		logger.debug("*** slc : {}", this.slc);
		int port = remotePeer.getPort();
		InetAddress remoteAddress;
		try {
			remoteAddress = InetAddress.getByName(ip != null ? ip : remotePeer
					.getFQDN());
		} catch (UnknownHostException e) {
			throw new TransportException("Can not found host",
					TransportError.Internal, e);
		}
		IContext actionContext = getContext();
		this.fsm = fsmFactory.createInstanceFsm(actionContext,
				concurrentFactory, config);
		this.fsm.addStateChangeNotification(new StateChangeListener() {
		...
		if (connection == null) {
			...
			if (portRange != null) {
				try {
					...
					if (local.getIPAddresses() != null
							&& local.getIPAddresses().length > 0) {
						localAddress = local.getIPAddresses()[0];
					} else {
						localAddress = InetAddress.getByName(metaData
								.getLocalPeer().getUri().getFQDN());
					}
				} catch (Exception e) {
				...
				try {
					...
					localPort = strRange
							+ new Random().nextInt(endRange - strRange + 1);
				} catch (Exception exc) {
				...
				if (logger.isDebugEnabled()) {
					...
					logger.debug(
							"Create conn with localAddress={}; localPort={}",
							localAddress, localPort);
				}
			}
			this.connection = trFactory.createConnection(remoteAddress,
					concurrentFactory, port, localAddress, localPort,
					connListener, ref);
		} else {
		...
		logger.debug("Peer created : {}", this.toString());
	}
	...
	public void addPeerStateListener(final PeerStateListener listener) {
		fsm.addStateChangeNotification(new StateChangeListener() {
			public void stateChanged(Enum oldState, Enum newState) {
				listener.stateChanged((PeerState) oldState,
						(PeerState) newState);
			}
	...
	private IMessage processRedirectAnswer(IMessage answer) {
		...
		try {
			...
			// Loop detected (may be stack must send error response to redirect
			// host)
	...
	public void connect() throws InternalException, IOException,
			IllegalDiameterStateException {
		if (getState(PeerState.class) != PeerState.DOWN) {
			throw new IllegalDiameterStateException("Invalid state:"
					+ getState(PeerState.class));
		}
		try {
	...
	public void disconnect() throws InternalException,
			IllegalDiameterStateException {
		if (getState(PeerState.class) != PeerState.DOWN) {
			stopping = true;
			try {
	...
	public IMessage[] remAllMessage() {
		IMessage[] m = peerRequests.values().toArray(
				new IMessage[peerRequests.size()]);
		peerRequests.clear();
		return m;
	}
	...
	public boolean sendMessage(IMessage message) throws TransportException,
			OverloadException, InternalException {
		if (PeerImpl._VALDIATOR_.isOn()) {
			PeerImpl._VALDIATOR_.validate(message, false);
		}
		return !stopping
				&& fsm.handleEvent(new FsmEvent(EventTypes.SEND_MSG_EVENT,
						message));
	}
	...
	public String toString() {
		return "Peer{" + "Uri=" + uri + "; State="
				+ fsm.getState(PeerState.class).toString() + "}";
	}
		...
		if (avps != null) {
			...
			for (int i = 0; i < avps.size(); i++) {
				try {
					...
					logger
							.warn("Can not get ip address from HOST_IP_ADDRESS avp");
				}
			}
	...
	protected Set<ApplicationId> getCommonApplicationIds(IMessage message) {
		...
		Set<ApplicationId> locAppId = metaData.getLocalPeer()
				.getCommonApplications();
		Set<ApplicationId> remAppId = message.getApplicationIdAvps();
		logger
				.debug(
						"Checking common applications. Remote applications: {}. Local applications: {}",
		...
		for (ApplicationId l : locAppId) {
			for (ApplicationId r : remAppId) {
				if (l.equals(r)) {
					newAppId.add(l);
				} else if (r.getAcctAppId() == INT_COMMON_APP_ID
						|| r.getAuthAppId() == INT_COMMON_APP_ID
						|| l.getAcctAppId() == INT_COMMON_APP_ID
						|| l.getAuthAppId() == INT_COMMON_APP_ID) {
					newAppId.add(r);
				}
			}
		}
		logger.debug("*** new AppId: {}", newAppId);
		return newAppId;
	}
	...
	protected class ActionContext implements IContext {
		...
		public void connect() throws InternalException, IOException,
			...
			try {
				...
				if (logger.isDebugEnabled()) {
					logger
							.debug("Connected to peer {}", PeerImpl.this
									.getUri());
				}
			} catch (TransportException e) {
				switch (e.getCode()) {
				case NetWorkError:
					throw new IOException("Can not connect to "
							+ connection.getKey() + " - " + e.getMessage());
		...
		public void disconnect() throws InternalException,
			...
			if (connection != null) {
				...
				if (logger.isDebugEnabled()) {
					logger.debug("Disconnected from peer {}", PeerImpl.this
							.getUri());
				}
			}
		}
		...
		public boolean sendMessage(IMessage message) throws TransportException,
			...
			logger.debug("Send message {} to peer {} ({})", new Object[] {
					message, PeerImpl.this.getUri(),
					message.getAvps().asArray() });
			return true;
		}

		public void sendCerMessage() throws TransportException,
				OverloadException {
			if (logger.isDebugEnabled()) {
			...
			IMessage message = parser.createEmptyMessage(
					CAPABILITIES_EXCHANGE_REQUEST, 0);
			message.setRequest(true);
			message.setHopByHopIdentifier(getHopByHopIdentifier());

			if (useUriAsFQDN) {
				message.getAvps().addAvp(ORIGIN_HOST,
						metaData.getLocalPeer().getUri().toString(), true,
						false, true);
			} else {
				message.getAvps().addAvp(ORIGIN_HOST,
						metaData.getLocalPeer().getUri().getFQDN(), true,
						false, true);
			}

			message.getAvps().addAvp(ORIGIN_REALM,
		...
		public void sendDwrMessage() throws TransportException,
				OverloadException {
			if (logger.isDebugEnabled()) {
				logger.debug("Send DWR message");
			}
			IMessage message = parser.createEmptyMessage(
					DEVICE_WATCHDOG_REQUEST, 0);
			message.setRequest(true);
			message.setHopByHopIdentifier(getHopByHopIdentifier());
			// Set content
			message.getAvps().addAvp(ORIGIN_HOST,
		...
		public void sendDwaMessage(IMessage dwr, int resultCode,
				String errorMessage) throws TransportException,
				OverloadException {
			if (logger.isDebugEnabled()) {
			...
			// Set content
			message.getAvps()
					.addAvp(RESULT_CODE, resultCode, true, false, true);
		...
		public void sendDprMessage(int disconnectCause)
			...
			message.getAvps().addAvp(ORIGIN_REALM,
					metaData.getLocalPeer().getRealmName(), true, false, true);
			message.getAvps().addAvp(DISCONNECT_CAUSE, disconnectCause, true,
					false);
			sendMessage(message);
		}

		public void sendDpaMessage(IMessage dpr, int resultCode,
				String errorMessage) throws TransportException,
				OverloadException {
			if (logger.isDebugEnabled()) {
			...
			try {
				...
				if (origHost == null || origRealm == null || vendorId == null) {
					if (logger.isWarnEnabled()) {
						...
						logger
								.warn("Incorrect CEA message (missing mandatory AVPs)");
					}
				} else {
					if (realmName == null) {
					...
					if (cai.size() > 0) {
						...
						logger
								.debug("CEA did not containe appId, therefore  set local appids to common-appid field");
						commonApplications.clear();
						commonApplications.addAll(metaData.getLocalPeer()
								.getCommonApplications());
					}
		...
		public boolean receiveMessage(IMessage message) {
			logger.debug("*** receive message {} ({})", message, message
					.isRequest());
			...
			if (message.isRequest()) {
				...
				// logger.debug("*** message.sid:{}", avpSessionId);
				if (avpSessionId != null) {
					...
					if (listener != null) {
						...
						IMessage answer = (IMessage) listener
						...
						if (answer != null) {
							try {
								...
								logger.warn("Can not send immediate answer {}",
										answer);
							}
						}
						statistic.getRecordByName(
				...
				IMessage request = peerRequests.remove(message
				...
				if (request != null && !request.isTimeOut()) {
					...
					if (message != null) {
						if (request.getEventListener() != null) {
							request.getEventListener().receivedSuccessMessage(
									request, message);
						} else {
							if (logger.isDebugEnabled()) {
								...
								logger
										.debug(
												"Can not call answer listener for request {} because listener not set",
												message);
							}
							statistic.getRecordByName(
									IStatistic.Counters.NetGenRejectedResponse
											.name()).inc();
						}

						isProcessed = true;
						statistic.getRecordByName(
								IStatistic.Counters.NetGenResponse.name())
								.inc();
					} else {
						statistic.getRecordByName(
								IStatistic.Counters.NetGenRejectedResponse
					...
					statistic.getRecordByName(
							IStatistic.Counters.NetGenRejectedResponse.name())
							.inc();
				}
			}
			return isProcessed;
		...
		protected void addAppId(ApplicationId appId, IMessage message) { // todo
			// duplicate
			// code
			// look
			// SessionImpl
			...
			if (appId.getVendorId() == 0) {
				if (appId.getAuthAppId() != 0) {
					message.getAvps().addAvp(AUTH_APPLICATION_ID,
							appId.getAuthAppId(), true, false, true);
				} else if (appId.getAcctAppId() != 0) {
					message.getAvps().addAvp(ACCT_APPLICATION_ID,
							appId.getAcctAppId(), true, false, true);
				}
			} else {
				message.getAvps().addAvp(SUPPORTED_VENDOR_ID,
						appId.getVendorId(), true, false, true);
				AvpSet vendorApp = message.getAvps().addGroupedAvp(
						VENDOR_SPECIFIC_APPLICATION_ID, true, false);
				vendorApp.addAvp(VENDOR_ID, appId.getVendorId(), true, false,
						true);
				if (appId.getAuthAppId() != 0) {
					vendorApp.addAvp(AUTH_APPLICATION_ID, appId.getAuthAppId(),
							true, false, true);
				}
				if (appId.getAcctAppId() != 0) {
					vendorApp.addAvp(ACCT_APPLICATION_ID, appId.getAcctAppId(),
							true, false, true);
				}
			}
		}
