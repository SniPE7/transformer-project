...
class TCPTransportClient implements Runnable {
	...
	private static final Logger logger = LoggerFactory
			.getLogger(TCPTransportClient.class);
	 ...
	 */
	TCPTransportClient(IConcurrentFactory concurrentFactory,
			TCPClientConnection parenConnection) {
		this.parentConnection = parenConnection;
		this.concurrentFactory = concurrentFactory;

		logger.debug("TCPTransportClient {}", parenConnection);
	}
	...
	public void initialize(Socket socket) throws IOException,
		...
		socketChannel.configureBlocking(true);
		destAddress = new InetSocketAddress(socket.getInetAddress(), socket
				.getPort());
	}
	...
	public void start() throws Exception {
		...
		if (getParent() == null) {
			throw new NotInitializedException(
					"No parent connection is set is set");
		}
	...
	public void run() {
		...
		try {
			while (!stop) {
				int dataLength = readData(buffer);
				if (dataLength == -1) {
					break;
				}
				buffer.flip();
	...
	protected int readData(ByteBuffer buffer) throws IOException {
		int rc = socketChannel.read(buffer);
		return rc;
	}
	...
	public void setDestAddress(InetSocketAddress address) {
		destAddress = address;
		logger.debug("Destination address is set to {} : {}", destAddress
				.getHostName(), destAddress.getPort());
	}
	...
	protected int writeData(ByteBuffer bytes) throws IOException {
		int rc = socketChannel.write(bytes);
		return rc;
	}
	...
	void append(byte[] data) {
		if (storage.position() + data.length >= storage.capacity()) {
			...
			tmp.put(tmpData);
			storage = tmp;
			logger.warn("Increase storage size. Current size is {}", storage
					.array().length);
		}
