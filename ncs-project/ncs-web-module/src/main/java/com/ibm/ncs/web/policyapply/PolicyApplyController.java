...
//			model = encodeCateName(model);		//not used
			...
			//Initialize device related contents listed in "Device/Port", "Unselected" list and "Selected" list
			...
			if(selecting!=null||unselected!=null){
				if(tcate.equals("9")){		// predef mib policy
					...
					try{
						...
						if(mpid!=null&&!mpid.equals("")){ //previously selected objects for this device
							long devidLong = 0;
						...
						if(selecting!=null){								
						 for(int i=0;i<selecting.length;i++){
							...
							if(pdm==null){
								...
								//update the record in t_devpol_map table when devid is already existed in it
								pdm.setMpid(Long.parseLong(mpid));								
...
//									 System.out.println("\t\t&&&&&&&&" + this.getClass().getName() + " delete to TLinepolMapDao: pk= " + pk + " dto=" + tm.toString());
				...
				if(tcate.equals("4")){		//port policy , ready, tested
...
//					System.out.println("Step 1: \tIn tcate.equals(16 mpid=" + mpid);
					...
					try{
						...
						if(ppid!=null&&!ppid.equals("")){ //previously selected objects devices
...
//							System.out.println("\t??????----???????/after  remove=============preSelected.size=" + preSelectedPort.size());
						}
						
						if(selecting!=null){	
						 for(int i=0;i<selecting.length;i++){
							String temp = selecting[i];
							long ptid;
							long devid;
							long pdmid;
							if(selecting[i].startsWith("device_")){
								...
								if(tdm==null){		
									//insert to t_devpol_map when no record about this devid in this table
									tdm = new TDevpolMap();
									tdm.setDevid(devid);
					...
					try{
						...
						if(mpid!=null&&!mpid.equals("")){ //previously selected objects
...
//								System.out.println("\t=============before remove=============preSelected.size=" + preSelected.size());
...
//								System.out.println("\t\t&&&&&&&&" + this.getClass().getName() + " inserted to TDevpolMapDao: " + tm.toString());
...
//								System.out.println("\t\t&&&&&&&&" + this.getClass().getName() + " updated to TDevpolMapDao: pk= " + pk + " dto=" + tm.toString());
...
//		System.out.println("^^^^^^^^^^^^^^^^^select Service =" + selectDevice + "\t selectSercie1=" + selectDevice1);
...
//		System.out.println("\t\tmpname=^^^" + mpname);
		...
		if(selectDevice!=null&&!selectDevice.equals("0")){ //has a valid selection in jsp, but not seleting "Please select a group"
	...
	private Map getNodeList(Map model) throws TGrpNetDaoException{
		//There are 6 fields in table T_GRP_NET they are gid,gname,supid,levels,description,unm
...
//		grpnet.put("names", names);
	...
	private Map getDevicesOfNodes(Map model) throws TDeviceInfoDaoException, TListIpDaoException, TDevpolMapDaoException{
		...
		List<TDeviceInfo> deviceinfoed = new ArrayList();
...
//		System.out.println("********in getDevicesOfNodes**********");
		try {
				if(selectNode != null  && ! selectNode.equals("") && ! selectNode.equals("0")){ //when a valid group is selected
//					System.out.println("\t in getDevicesOfNodes selectNode="+selectNode);
...
//					System.out.println("\tgid="+agid);
					...
					if(deviceinfo!=null){
						for(int i = 0;i<deviceinfo.size();i++){
								...
								//find the devices that already defined policy for it, and
								// remove it from the selectable list
								...
								if(tm!=null){
									if(cate!= null && !cate.equalsIgnoreCase("") && !cate.equalsIgnoreCase("16")){
										...
										if(tm.getMpid()== mpidLong){
											...
											if(tpbase != null){
//												System.out.println("getDevicesOfNodes TPolicyBase mpid=" + tpbase.getMpid() + " \t mpname is" + tpbase.getMpname());
												policyApplied.put(tm.getDevid(), tpbase.getMpname());
											}
										}
									}else if(cate!= null && cate.equalsIgnoreCase("16")){
										long ppidLong = 0;
										if(ppid != null) ppidLong = Long.parseLong(ppid);
										if(tm.getPpid() == ppidLong){
											...
											if(tpbase != null){
//												System.out.println("getDevicesOfNodes TPolicyPeriodDao ppid=" + tpbase.getPpid() + " \t mpname is" + tpbase.getPpname());
					...
					//to be confirmed: what will be listed in "Device/Port" selection and  selected list
					if(selectDevice!=null && !selectDevice.equals("0")&&!selectDevice.equals("")){//if device/prot is selected
					...
					model.put("devicetmp", deviceinfo);		//confirmed: display all the IPs in this group
					/*model.put("deviceinfoed", deviceinfoed);
					model.put("deviceinfo", devtmp);*/	
				}
		} catch (NumberFormatException e) {
						...
						if(cate!= null && !cate.equalsIgnoreCase("") && !cate.equalsIgnoreCase("16")){
							...
							if(tm.getMpid()== mpidLong){
								...
								try {
									...
									if(tpbase != null){
//										System.out.println("getPortsOfDevice TPolicyBase ptid=" + tm.getPtid() + " \t mpname is" + tpbase.getMpname());
							...
							if(tm.getPpid() == ppidLong){
								...
								try {
									...
									if(tpbase != null){
//										System.out.println("getPortsOfDevice TPolicyBase ptid=" + tm.getPtid() + " \t mpname is" + tpbase.getPpname());
						...
						if(cate!= null && !cate.equalsIgnoreCase("") && !cate.equalsIgnoreCase("16")){
							...
							if(tm.getMpid()== mpidLong){
								...
								try {
									...
									if(tpbase != null){
//										System.out.println("getPdmsOfDevice TPolicyBase mpid=" + tpbase.getMpid() + " \t mpname is" + tpbase.getMpname());
							...
							if(tm.getPpid() == ppidLong){
								...
								try {
									...
									if(tpbase != null){
//										System.out.println("getPdmsOfDevice TPolicyBase mpid=" + tpbase.getPpid() + " \t mpname is" + tpbase.getPpname());
	...
	private List<TPortInfo> getPreSelectedPorts(long devidLong, String cate, long idLong){
		...
		List<TPortInfo> preSelected = new ArrayList();
		if (portinfo!= null)
		for(int i=0;i<portinfo.size();i++){
