...
public class DeleteIpnetOfGidController implements Controller {
	...
	private ModelAndView deleteTheNode(Map<String, Object> model)   {
		...
		try{
			...
			try {			
				...
				if (flag.equals("1")){
					Log4jInit.ncsLog.error(this.getClass().getName() + " Not Allowed to delete,  this unmalloced node directly");
				...
				if( tmpdata.size() >1){
					Log4jInit.ncsLog.error(this.getClass().getName() + " Not Allowed to delete, It may contains sub node! tmpdata=" + tmpdata.toString());
					...
					Log4jInit.ncsLog.info(this.getClass().getName() + " deleted from TGrpNetDao: " + pk.toString());
					
					TGrpNetDao.delete(pkunmalloc);
					Log4jInit.ncsLog.info(this.getClass().getName() + " deleted from TGrpNetDao: " + pkunmalloc.toString());
					
					TListIpDao.deleteByGid(gid);
					Log4jInit.ncsLog.info(this.getClass().getName() + " deleteByGid from TListIpDao: gid=" + gid);
		...
		try {
			...
			if ( gidL !=null && ! "".equals(gidL) ){
				...
				try {
					...
					Log4jInit.ncsLog.error(this.getClass().getName() + "Exception on Node info update, Get pkid error!!");
			...
			listip00.setCategory(1);	//??
				...
				Log4jInit.ncsLog.info(this.getClass().getName() + " Inserted to TListIpDao: " + listip00.toString());
			...
			Log4jInit.ncsLog.error(this.getClass().getName() + " Exception on Node info update!");
			
			throw new RuntimeException("Exception on Node info update!! ");
		} catch (TListIpDaoException e) {
			Log4jInit.ncsLog.error(this.getClass().getName() + " Exception on node info update , iplist update");
