...
public class DeleteSyslogNSController implements Controller {
	...
	public ModelAndView handleRequest(HttpServletRequest request,
		...
		System.out.println("after remove session content is************"+session.getAttribute("pagelistns"));
	...
	//		String mark = marks[i];
			try{
	//			List<SyslogEventsProcessNs> syslogs = syslogEventsProcessNsDao.findWhereMarkEquals(mark);
	//			System.out.println("syslogs is-----"+syslogs);
	//			if(syslogs != null){
	//				if(syslogs.size()>0){
	//					SyslogEventsProcessNs syslogtemp = syslogs.get(0);
		                SyslogEventsProcessNs syslogtemp = syslogEventsProcessNsDao.findByPrimaryKey(mark, manufacture);
						System.out.println("syslogtemp is----"+syslogtemp);
						syslogEventsProcessNsDao.delete(syslogtemp);
	...
	//				}
		//		}
				Log4jInit.ncsLog.info(this.getClass().getName() + " Deleted record from syslogEventsProcessNsDao " + mark);
