...
function upload(){
   form1.action="<%=request.getContextPath() %>/secure/excel/excel2db.wss?checkmode=SYSLOG";
   this.form1.submit();
       document.getElementById("errmsg").innerHTML = "<img border='0' src='../../images/icon_progress.gif' width='16' height='16' > 开始增量导入，请稍候....";
}

function uploadAll(){
//   from1.type.value='all';
   form1.action="<%=request.getContextPath() %>/secure/excel/excel2dballsyslog.wss";
   this.form1.submit();
 	document.getElementById("errmsg").innerHTML = "<img border='0' src='../../images/icon_progress.gif' width='16' height='16' > 开始备份...全量导入，请稍候....";   
   
}

function backup(){
    form1.action="<%=request.getContextPath() %>/secure/excel/backupsyslog.wss";
   this.form1.submit();
   document.getElementById("errmsg").innerHTML = "<img border='0' src='../../images/icon_progress.gif' width='16' height='16' > 开始备份，请稍候....";
}

function restorepre(){
   form1.action="<%=request.getContextPath() %>/secure/excel/restoresyslogpre.wss";
   this.form1.submit();
}
</script>
</head>
...
<table border="0" cellpadding="3" cellspacing="0" valign="top" width="100%" summary="Framing Table" CLASS="button-section">
    ...
    <table style="display: inline; font-size: 95%;" cellspacing="0" cellpadding="0" border="0"><tr> 
    	...
    	<td>
    		<input type="button" name="button.update" value="备份SYSLOG" class="buttons" id="functions" onClick="backup()"/>
    	</td>
    	
    	 <td>
    		<input type="button" name="button.update" value="恢复以前SYSLOG" class="buttons" id="functions" onClick="restorepre()"/>
    	</td>
...
<table border="0" cellpadding="3" cellspacing="1" valign="top" width="100%" summary="Framing Table" CLASS="button-section">
        ...
        <td class="table-row" nowrap> 
            ...
            <c:if test="${model.Message != null &&  model.Message != ''}"><fmt:message>${model.Message }</fmt:message><br/></c:if>
            <c:if test="${model.messageforns != null &&  model.messageforns != ''}"><fmt:message>${model.messageforns }</fmt:message></c:if>
...
<TABLE WIDTH="98%" CELLPADDING="0" CELLSPACING="0" BORDER="0" class="portalPage" id="syslogListTab">
 <TBODY ID="wasUniPortlet">
    <TR>   
  		<TD CLASS="wpsPortletArea" COLSPAN="3" >    
	                 
	        <h3 id="title-bread-crumb">Syslog列表</h3>
			<!-- Data Table -->
							...
							<table BORDER="0" CELLPADDING="3" CELLSPACING="1" WIDTH="100%" SUMMARY="List table" CLASS="framing-table">
								...
								<tr class="table-row" ondbclick="choise(this);">
									<td VALIGN="middle"  class="collection-table-text">
                                    	<a href="<%=request.getContextPath() %>/secure/syslog/syslogEventsProcess.wss">Syslog Event Process</a>
                                    </td>
								</tr>
                                <tr class="table-row" ondbclick="choise(this);">
									<td VALIGN="middle"  class="collection-table-text">
                                    	<a href="<%=request.getContextPath() %>/secure/syslog/syslogEventsProcess_NS.wss">Syslog Event Process NS</a>
                                    </td>
								</tr>
