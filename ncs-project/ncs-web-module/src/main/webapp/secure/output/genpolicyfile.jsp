...
function exeShell(){
   ...
   Dframe.location.href="<%=request.getContextPath()%>/secure/policyapply/exeshell.wss";
        ...
        <table border="0" cellpadding="3" cellspacing="0" valign="top" width="100%" summary="Framing Table" class="button-section">
          ...
          <tr valign="top">
            ...
            <table style="display: inline; font-size: 95%;" cellspacing="0" cellpadding="0" border="0">
              <tr>
                <td><input type="button" value="生成监控配置"  onclick="javascript:applyPolicy()"  name="apply" id="apply" class="buttons">
                 <input type="button" value="生效"  onclick="javascript:exeShell()"  name="exeshell" id="exeshell" class="buttons">
<!--                <input type="button" value="生效摘要"  onclick="javascript:downloadAppPolicylylog()" class="button" name="downapplypolicylog">-->
                </td>
...
<c:forEach items="${model.stat}" var="msg"><c:out value="${msg.key}"/>&nbsp;<c:out value="${msg.value}"/><br></c:forEach>
</div>
<div id=info2 class="table-row">
<c:forEach items="${model.stat}" var="msg"><c:out value="${msg.key}"/>&nbsp;<c:out value="${msg.value}"/><br></c:forEach>
</div>
</font>
</td></tr></table>
