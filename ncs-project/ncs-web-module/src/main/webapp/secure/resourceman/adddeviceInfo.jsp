...
function choosedevice(){
	newdev.action = "<%=request.getContextPath() %>/secure/resourceman/chooseDevicetype.wss?formAction=add&gid="+newdev.gid.value+"&supid="+newdev.supid.value+"&level="+newdev.level.value;
	//alert("in add device info.jsp newdev.action="+newdev.action);
	newdev.submit();
}
...
function adddeviceinfo(){
	var v = confirm('确定保存此信息?');
	if( ! v ){ return false;	}
	newdev.formAction.value= 'addnew';
	newdev.action = "<%=request.getContextPath() %>/secure/resourceman/newdeviceInfo.wss";
	newdev.submit();
	return true;
}
function cancel(){
...
function pollingfunc(){
	...
	if(v){
		newdev.formAction.value = "new";
		newdev.action="<%=request.getContextPath()%>/secure/resourceman/pollsnmp.wss?formAction=add";
		newdev.submit();
	}
}
...
<TABLE WIDTH="100%" CELLPADDING="0" CELLSPACING="0" BORDER="0">
  ...
  <TD CLASS="wpsPortletArea" COLSPAN="3" >
        ...
        <h1 id="title-bread-crumb">添加新设备</h1>
...
<!-- button section --><br>
    ...
    <table style="display: inline; font-size: 95%;" cellspacing="0" cellpadding="0" border="0"><tr> 
		...
		<td>
    		<input type="button" name="button.update" value="确认" onClick="adddeviceinfo()"  class="buttons" id="functions"/>
    	</td>
		<td>
	 ...
	 <c:if test="${model.message != null &&  model.message != ''}">
									<div id="errmsg"><fmt:message key="${model.message }" /></div>
	</c:if>
...
<!--								</tr>-->
								<tr class="table-row">
									...
									<td><input  type="button" name="selDeviceType" value="选择类型" 
												onclick="choosedevice();" class="buttons" id="buttonStyle1">
                                    </td>
								</tr>
								<tr class="table-row">
									<td>子类型</td>
								...
								<tr class="table-row">
									...
									<td><input id="snmp" type="hidden" name="dtid2" value="${model.form.dtid }"/></td>
								</tr>
								<tr class="table-row">
									<td>共同体名</td>
...
<c:catch>
				<table cellspacing="1" cellpadding="3" border="0" width="100%" class="framing-table">
					...
					<tr class="table-row"><td colspan="3">
							<table id="flashs" border="0" cellspacing="1" cellpadding="3" border="0" width="100%" class="framing-table">
								<tbody>
									<tr class="table-row">
										<td>&nbsp;</td>
						...
						<c:forEach items="${model.form.flashfile}"	var="ff" varStatus="y" begin="0" step="1"	>	
						<tr class="table-row">
