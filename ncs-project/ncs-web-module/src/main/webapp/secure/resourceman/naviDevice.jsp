...
<table height="26" width="100%" cellspacing="0" cellpadding="0"
	...
	<tr>
		<td align="left" colspan="1" class="navtree"
			style="background-color: #FFFFFF">
		<!-- title of the navi welcome page -->
		<ul class='nav-child' dir='ltr'>
			<li class='navigation-bullet'>设备</li>
		</ul>
		</td>
        <td align="right"><ul class='nav-child' dir='ltr'>
...
<% nodei++ ;%>
...
</td></tr><tr><td align="left" colspan="2" class="navtree"
			style="background-color: #FFFFFF">
		...
		<div nowrap class='main-task' style='margin-left: 0.3em;'>
			<img src='<%=request.getContextPath() %>/images/arrow_expanded.gif' 
				 ...
				 href = "<%= request.getContextPath() %>/secure/resourceman/addNewDevice.jsp"
				...
				<c:forEach  items="${model.ipnettree[theRoot]}" var="theL0"  > 
					...
					<% nodei++; %>
   					<div nowrap class='sub-task'>
                        ...
                        <a style='color:#000000;text-decoration:none;' 
                        	href="<%=request.getContextPath() %>/secure/resourceman/deviceInfoOfGid.wss?gid=${model.names[theL0.key].gid}"
                            target="detail" title="${model.names[theL0.key].gname}">    				
                            ${model.names[theL0.key].gname}
                         </a>
  				 					...
  				 					<c:when test="${model.names[theL1.key].unmallocedipsetflag eq 0}">
  										<div nowrap class='sub-task'>
                                            <img src='<%=request.getContextPath() %>/images/arrow_expanded.gif' 
                                                ...
                                                title='Collapse' alt='Collapse' align='absmiddle' id='I<%=nodei %>' border='0'/>
                                            <a style='color:#000000;text-decoration:none;'
                                                href="<%=request.getContextPath() %>/secure/resourceman/deviceInfoOfGid.wss?gid=${model.names[theL1.key].gid}"
                                                target="detail" 
                                                title="${model.names[theL1.key].gname}">
                                                ${model.names[theL1.key].gname}</a>
			    						</div>
			    					</c:when>
                                   ...
                                   <c:choose><c:when test="${fn:length(model.ipnettree[theL1.key]) >1}"> 
    									<% nodei++; %>
  				 						...
  				 						<c:when test="${model.names[theL2.key].unmallocedipsetflag eq 0}">
  											<div nowrap class='third-child-container' style='display:block' >	
                                                <img src='<%=request.getContextPath() %>/images/arrow_expanded.gif' 
                                                    onClick="javascript:expandCollapse('<%=nodei %>');"
                                                    title='Collapse' alt='Collapse' align='absmiddle' id='I<%=nodei %>' border='0'/>
                                                <a style='color:#000000;text-decoration:none;'
                                                    
                                                    href="<%=request.getContextPath() %>/secure/resourceman/deviceInfoOfGid.wss?gid=${model.names[theL2.key].gid}"
                                                    target="detail" 
                                                    title="${model.names[theL2.key].gname}">
                                                    ${model.names[theL2.key].gname}</a>
			    							</div>
			    						</c:when>
                                        ...
                                        <c:choose><c:when test="${fn:length(model.ipnettree[theL2.key]) >1}">     
  												...
  												<% nodei++; %>
  				 								...
  				 								<c:when test="${model.names[theL3.key].unmallocedipsetflag eq 0}">
  													<div class='fourth-child-container' style='display:block'>	
                                                        <img src='<%=request.getContextPath() %>/images/arrow_expanded.gif' 
                                                            onClick="javascript:expandCollapse('<%=nodei %>');"
                                                            title='Collapse' alt='Collapse' align='absmiddle' id='I<%=nodei %>' border='0'/>
			    										<a style='color:#000000;text-decoration:none;'
   															href="<%=request.getContextPath() %>/secure/resourceman/deviceInfoOfGid.wss?gid=${model.names[theL3.key].gid}"
                                                            target="detail" 
                                                            title="${model.names[theL3.key].gname}">
                                                            ${model.names[theL3.key].gname}</a>
			    									</div>
			    								</c:when>
                                                 ...
                                                 <c:choose><c:when test="${fn:length(model.ipnettree[theL3.key]) >1}"> 
														...
														<% nodei++; %>
                                 						...
                                 						<c:when test="${model.names[theL4.key].unmallocedipsetflag eq 0}">
  															<div nowrap class='fifth-child-container' style='display:block' >	
                                                                <img src='<%=request.getContextPath() %>/images/arrow_expanded.gif' 
                                                                    onClick="javascript:expandCollapse('<%=nodei %>');"
                                                                    title='Collapse' alt='Collapse' align='absmiddle' id='I<%=nodei %>' border='0'/>
                                                                <a style='color:#000000;text-decoration:none;'
                                                                    
                                                                    href="<%=request.getContextPath() %>/secure/resourceman/deviceInfoOfGid.wss?gid=${model.names[theL4.key].gid}"
                                                                    target="detail" 
                                                                    title="${model.names[theL4.key].gname}">
                                                                    ${model.names[theL4.key].gname}</a>
			    											</div>
			    										</c:when>
                                						...
                                						<!-- L5 more levels  -->
                                                        <div class='sixth-child-container' style='display:block' id='N<%=nodei %>' >				    				
                                                           ...
                                                           <c:when test="${model.names[theL5.key].unmallocedipsetflag eq 0}">
                                                                <div nowrap class='sixth-child-container' style='display:block' >	
                                                                    ...
                                                                    <a style='color:#000000;text-decoration:none;'
                                                                        ...
                                                                        href="<%=request.getContextPath() %>/secure/resourceman/deviceInfoOfGid.wss?gid=${model.names[theL5.key].gid}"
                                                                        target="detail" 
                                                                        title="${model.names[theL5.key].gname}">
                                                                        ${model.names[theL5.key].gname}</a>
                                                                </div>
                                                           </c:when>
