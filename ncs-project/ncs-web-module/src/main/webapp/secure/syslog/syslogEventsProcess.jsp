...
function add(){
	...
	form1.action = "<%=request.getContextPath() %>/secure/syslog/syslog.wss";
	this.form1.submit();
}
function deleteform1(){
	...
	form1.action = "<%=request.getContextPath() %>/secure/syslog/deletesyslog.wss";
	this.form1.submit();
}
...
<TABLE WIDTH="100%" CELLPADDING="0" CELLSPACING="0" BORDER="0">
  ...
  <TD CLASS="wpsPortletArea" COLSPAN="3" >
        ...
        <h1 id="title-bread-crumb">Syslog Event Process 列表</h1>
        <p ></p>
...
<th NOWRAP VALIGN="BOTTOM" CLASS="table-sortable:default column-head-name" SCOPE="col" width="20%" style="word-break:break-all">Status</th>
...
<c:url value="/secure/syslog/updateToSyslogEventProcess.wss" var="markmanuf">
...
&nbsp;
<a href="<%=request.getContextPath() %>/secure/syslog/deletesyslog.wss?mark=${c1.mark}&manufacture=${c1.manufacture}" onclick="return confirm('确定删除信息?');">
			<img border="0" src="../../images/delete.gif" width="16" height="16" alt="删除"></a>
</td>
<td VALIGN="top"  class="collection-table-text" width="220" style="word-break:break-all"><c:out value='${c1.origevent }'/></td>
...
<c:if test="${! model.pagedtabList.firstPage }">
  <a href="<%=request.getContextPath() %>/secure/syslog/syslogEventsProcess.wss?cmd=firstPage">[首页</a>/
  <a href="<%=request.getContextPath() %>/secure/syslog/syslogEventsProcess.wss?cmd=previousPage">上一页]</a>
</c:if>
<c:forEach var="sequence" begin="${model.startpage }" end="${model.endpage }">
   <a href="<%=request.getContextPath() %>/secure/syslog/syslogEventsProcess.wss?page=${sequence - 1}" >${sequence }</a>
</c:forEach>
<c:if test="${! model.pagedtabList.lastPage }">
  <a href="<%=request.getContextPath() %>/secure/syslog/syslogEventsProcess.wss?cmd=nextPage">[下一页</a>/
  <a href="<%=request.getContextPath() %>/secure/syslog/syslogEventsProcess.wss?cmd=lastPage">末页]</a>
</c:if>
