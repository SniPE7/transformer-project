 ...
 *
 */
public class ClientCertAuditEventRecognizer extends AbstractAuditEventRecognizer {

  /**
   * 
   */
  public ClientCertAuditEventRecognizer() {
    super();
  }
  ...
  protected boolean isAuditEvent(LoginModuleEvent event) {
    return event.getPhase().name().equals("LOGIN") 
        && event.getLoginModuleEntry().getLoginModuleName().equals(ClientCertLoginModule.class.getName());
  }
  ...
  protected void completeAuditEvent(LoginModuleEvent event, W7Event w7Event) {
    w7Event.getWhat().setVerb(readValue(ClientCertLoginModule.class.getSimpleName()));
  }
