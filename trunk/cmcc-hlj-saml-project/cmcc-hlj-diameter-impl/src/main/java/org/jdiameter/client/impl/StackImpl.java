...
public class StackImpl implements IContainer, StackImplMBean {
	...
	protected DiameterMessageValidator validator = DiameterMessageValidator
			.getInstance();
	/**
	...
	public SessionFactory init(Configuration config)
		...
		try {
			...
			try {
				Class assemblerClass = Class.forName(config.getStringValue(
						Assembler.ordinal(), (String) Assembler.defValue()));
				assembler = (IAssembler) assemblerClass.getConstructor(
						Configuration.class).newInstance(config);
			...
			this.config = config;
			// created manager
			this.peerManager = (IPeerTable) assembler
					.getComponentInstance(IPeerTable.class);
			// log.debug("*** peerTable: {}",peerManager);
			this.concurrentFactory = (IConcurrentFactory) assembler
					.getComponentInstance(IConcurrentFactory.class);
			this.peerManager.setAssembler(assembler);
			this.state = StackState.CONFIGURED;
		} finally {
			lock.unlock();
		}
		return (SessionFactory) assembler
				.getComponentInstance(SessionFactory.class);
	}
	...
	public void start() throws IllegalDiameterStateException, InternalException {
		...
		try {
			...
			scheduledFacility = concurrentFactory
					.getScheduledExecutorService(ProcessingMessageTimer.name());
	...
	public void start(final Mode mode, long timeOut, TimeUnit timeUnit)
		...
		try {
			...
			final CountDownLatch barrier = new CountDownLatch(Mode.ANY_PEER
					.equals(mode) ? 1 : peerTable.size());
	...
	public void stop(long timeOut, TimeUnit timeUnit)
		...
		try {
			if (state == StackState.STARTED || state == StackState.CONFIGURED) {
				List<Peer> peerTable = peerManager.getPeerTable();
				final CountDownLatch barrier = new CountDownLatch(peerTable
						.size());
				StateChangeListener listener = new StateChangeListener() {
					public void stateChanged(Enum oldState, Enum newState) {
				...
				assembler.getComponentInstance(IStatisticProcessor.class)
						.stop();
				try {
					if (peerManager != null) {
						peerManager.stopped();
	...
	public boolean isWrapperFor(Class<?> aClass) throws InternalException {
		...
		if (!isWrap) {
			isWrap = assembler.getChilds()[StackLayer.id()]
					.getComponentInstance(aClass) != null;
		}
		if (!isWrap) {
			isWrap = assembler.getChilds()[ControllerLayer.id()]
					.getComponentInstance(aClass) != null;
		}
		if (!isWrap) {
			isWrap = assembler.getChilds()[TransportLayer.id()]
					.getComponentInstance(aClass) != null;
		}
		return isWrap;
	}
	...
	public <T> T unwrap(Class<T> aClass) throws InternalException {
		...
		if (unwrapObject == null) {
			unwrapObject = assembler.getChilds()[StackLayer.id()]
					.getComponentInstance(aClass);
		}
		if (unwrapObject == null) {
			unwrapObject = assembler.getChilds()[ControllerLayer.id()]
					.getComponentInstance(aClass);
		}
		if (unwrapObject == null) {
			unwrapObject = assembler.getChilds()[TransportLayer.id()]
					.getComponentInstance(aClass);
		}
	...
	public void sendMessage(IMessage message) throws RouteException,
			AvpDataException, IllegalDiameterStateException, IOException {
		peerManager.sendMessage(message);
	}

	public void addSessionListener(String sessionId, NetworkReqListener listener) {
		log.debug("*** add session listener: {}|{}", sessionId, listener);
