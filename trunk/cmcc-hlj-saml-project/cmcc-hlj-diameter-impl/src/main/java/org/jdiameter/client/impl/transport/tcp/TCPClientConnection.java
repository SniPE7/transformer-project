...
import java.util.concurrent.ConcurrentLinkedQueue; // FIXME : requires JDK6 : import java.util.concurrent.LinkedBlockingDeque;
...
public class TCPClientConnection implements IConnection {
	...
	private static Logger log = LoggerFactory
			.getLogger(TCPClientConnection.class);
	...
	protected LinkedBlockingQueue<Event> buffer = new LinkedBlockingQueue<Event>(
			64);
	...
	public TCPClientConnection(Configuration config,
	...
	public void connect() throws TransportException {
		try {
			...
			throw new TransportException("Cannot init transport: ",
					TransportError.NetWorkError, e);
		} catch (Exception e) {
			throw new TransportException("Cannot init transport: ",
	...
	public void disconnect() throws InternalError {
		try {
			...
			throw new InternalError("Error while stopping transport: "
					+ e.getMessage());
		}
	}
	...
	public void sendMessage(IMessage message) throws TransportException,
			OverloadException {
		try {
			if (getClient() != null) {
			...
			throw new TransportException("Cannot send message: ",
					TransportError.FailedSendMessage, e);
		}
	}
	...
	public String getKey() {
		if (this.cachedKey == null) {
			this.cachedKey = new StringBuffer("aaa://").append(
					getRemoteAddress().getHostName()).append(":").append(
					getRemotePort()).toString();
		}
	...
	protected void onMessageReveived(ByteBuffer message)
			...
			if (processBufferedMessages(event)) {
				for (IConnectionListener listener : listeners) {
					...
					case MESSAGE_RECEIVED:
						listener.messageReceived(getKey(), parser
								.createMessage(event.message));
						break;
					case DATA_EXCEPTION:
						listener.internalError(getKey(), null,
								new TransportException("Avp Data Exception:",
										TransportError.ReceivedBrokenMessage,
										event.exception));
						break;
					}
				}
	...
	protected boolean processBufferedMessages(Event event)
			throws AvpDataException {
		if (listeners.size() == 0) {
			try {
				buffer.add(event);
