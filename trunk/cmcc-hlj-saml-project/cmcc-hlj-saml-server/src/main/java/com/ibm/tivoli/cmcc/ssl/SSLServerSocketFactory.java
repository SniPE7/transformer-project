...
public class SSLServerSocketFactory extends javax.net.ServerSocketFactory {
  ...
  public static javax.net.ServerSocketFactory getServerSocketFactory(String protocol, String keyManagerAlgorithm, InputStream keyStoreInputStream, String keyStoreType,
      char[] storePassword, char[] keyPassword) throws IOException {
    if (isSslEnabled()) {
      if (sslFactory == null) {
        try {
          sslFactory = SSLContextFactory.getServerInstance(protocol, keyManagerAlgorithm, keyStoreInputStream, keyStoreType, storePassword, keyPassword).getServerSocketFactory();
