...
//					"select c.mrid,a.dtid,a.category,c.mrname,a.name,a.subcategory from t_manufacturer_info_init c left join (select * from t_device_type_init d,t_category_map_init b where d.category=b.id) a on a.mrid=c.mrid order by c.mrName)", this);
	...
	/**
	 * Method 'listDeviceTypeTree';mrid,category,mrname,name,subcategory , model, objectid
	 * 
	 * @throws DeviceTypeTreeDaoException
	 * @return List<DeviceTypeTree>
	 */
...
//					"select c.mrid,a.dtid,a.category,c.mrname,a.name,a.subcategory from t_manufacturer_info_init c left join (select * from t_device_type_init d,t_category_map_init b where d.category=b.id) a on a.mrid=c.mrid order by c.mrName)", this);
	...
	{
		try {
//			return jdbcTemplate.query("select c.mrid,a.dtid,a.category,c.mrname,a.name,a.subcategory from t_manufacturer_info_init c " +
//					"left join (select * from t_device_type_init d,t_category_map_init b where d.category=b.id ) a on a.mrid=c.mrid " +
//					" where mrname='"+mrname+"'"
//					, this);
...
//			return jdbcTemplate.query("select c.mrid,a.dtid,a.category,c.mrname,a.name,a.subcategory from t_manufacturer_info_init c " +
//					"left join (select * from t_device_type_init d,t_category_map_init b where d.category=b.id ) a on a.mrid=c.mrid " +
//					" where  mrname=' "+mrname +"' and a.name='"+category +"' orderby mrname"
//					, this);
			return jdbcTemplate.query("select c.mrid,a.dtid,a.category,c.mrname,a.name,a.subcategory , a.model,a.objectid,a.logo,a.description from t_manufacturer_info_init c  " +
					" join (select * from t_device_type_init d,t_category_map_init b where d.category=b.id) a  " +
					"on a.mrid=c.mrid  " +
					"where mrname=? and    a.name=? ", this,mrName,cateName);
		}
		catch (Exception e) {
			throw new DeviceTypeTreeDaoException("Query failed", e);
		}
...
//					, this);
	...
	public List<DeviceTypeTree> searchByMulti(String mrName, String cateName,
			String subcate, String model, String objectid, String descr)
			throws DeviceTypeTreeDaoException {
		    StringBuffer sbbase = new StringBuffer("select c.mrid,a.dtid,a.category,c.mrname,a.name,a.subcategory , a.model,a.objectid,a.logo,a.description from t_manufacturer_info_init c  " +
					" join (select * from t_device_type_init d,t_category_map_init b where d.category=b.id)  a " +
					"on a.mrid=c.mrid  " );
		...
		public DeviceTypeTree findByDtid(long dtid) throws DeviceTypeTreeDaoException{
			...
			try{
				list = jdbcTemplate.query("select c.mrid,a.dtid,a.category,c.mrname,a.name,a.subcategory , a.model,a.objectid,a.logo,a.description from t_manufacturer_info_init c  " +
						" join (select * from t_device_type_init d,t_category_map_init b where d.category=b.id)  a " +
						"on a.mrid=c.mrid  and dtid=?" , this,dtid);
