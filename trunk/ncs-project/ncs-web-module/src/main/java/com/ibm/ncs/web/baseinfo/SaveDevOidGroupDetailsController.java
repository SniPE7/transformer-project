...
public class SaveDevOidGroupDetailsController implements Controller {
				...
				if(oids != null && oids.size()>0){
				  for(int i=0;i<oids.size();i++){
					if(Long.parseLong(oidindexStr) == oids.get(i).getOidindex()){
						System.out.println("new dupindex----");
...
//				System.out.println("@@==else: output DTO in update======@@:" + dto1);
				...
				if(oids != null && oids.size()>0){
				  for(int i=0;i<oids.size();i++){
					  System.out.println("opid list oidname is "+oids.get(i).getOidname()+" and the opid now is "+oidname);
					if(!oids.get(i).getOidname().equalsIgnoreCase(oidname)){
						System.out.println("not now record----");
						System.out.println("oidname is "+oids.get(i).getOidname()+" and oidindexStr is "+oidname);
					if(Long.parseLong(oidindexStr) == oids.get(i).getOidindex()){
						message = "baseinfo.oidgroup.dulindex";
						model.put("message", message);
