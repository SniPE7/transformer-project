...
public class UpdateSyslogNSController implements Controller{
	String pageView;
	String Message = "";
...
//				attentionflag+"--events--"+events+"--origevent--"+origevent+"--manufacture--"+manufacture+"--notcareflag--"+notcareflag);
		//validate time
		try{
		...
		SyslogEventsProcessNs syslogs = new SyslogEventsProcessNs();
		try{
			if(mark != null){
				syslogs = syslogEventsProcessNsDao.findByPrimaryKey(markOld, manuf);
			...
			if(syslogs != null){
				...
				if(!manuf.equalsIgnoreCase(manufacture) || !mark.equalsIgnoreCase(markOld)){
					SyslogEventsProcessNs syslogsTmp = syslogEventsProcessNsDao.findByPrimaryKey(mark, manufacture);
					if(syslogsTmp !=null){
						...
						syslogEventsProcessNsDao.delete(syslogs);
						Log4jInit.ncsLog.info(this.getClass().getName() + "Deleted from syslogEventsProcessNsDao:\n" + syslogs);
					...
					try{
					syslogEventsProcessNsDao.update(syslog);	
				    tabList = syslogEventsProcessNsDao.findAll();
				    pagedList.setSource(tabList);
					Log4jInit.ncsLog.info(this.getClass().getName() + "Updated to syslogEventsProcessNsDao:\n" + syslog);
					}catch(Exception e){
...
//	public  long stringToTime(String s){
//		StringTokenizer st = new StringTokenizer(s, ":");
//		String hoursStr   = st.nextToken();
//		String minutesStr = st.nextToken();
//		
