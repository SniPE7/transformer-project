...
<c:forEach  items="${model.mfTree}" var="theManufacturer" >
   ...
   <div nowrap class='sub-task'>
    			...
    			<a style='color:#000000;text-decoration:none;' 
    				onClick="javascript:expandCollapse('<%=mfsubnodei %>');"
    				href="<%=request.getContextPath() %>/secure/baseinfo/funcspec.wss?mf=${theManufacturer.key}" 
    				target="detail" 
    			...
    			<div class='sub-child-container' style='display:block' id='N<%=mfsubnodei %>'>	
  				...
  				<div nowrap class='sub-task'>
			    			...
			    			<a style='color:#000000;text-decoration:none;'
   								onClick="javascript:expandCollapse('<%=mfsubnodei %>');" 
			    				href="<%=request.getContextPath() %>/secure/baseinfo/funcspec.wss?mf=${theManufacturer.key}&cate=${theCategory.key}"
			    				target="detail" 
					    		...
					    		<a style='text-decoration:none' 
					    			href="<%=request.getContextPath() %>/secure/baseinfo/funcspec.wss?mf=${theManufacturer.key}&cate=${theCategory.key}&subcate=${theSubCategory.key}" 
					    			target="detail" 
					    			dir="ltr" 
