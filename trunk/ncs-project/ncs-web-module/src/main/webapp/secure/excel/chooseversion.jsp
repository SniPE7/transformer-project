...
function restore(){
	 ...
	 if( ! v ){ return false;	}
	 if(v){
	     form1.action = "<%=request.getContextPath() %>/secure/excel/restore.wss?checkmode=${model.mode}";
	     this.form1.submit();
	}
}
...
function deleteversion(){
 var v = confirm('确定要删除该版本吗?');
	 if( ! v ){ return false;	}
	 if(v){
	     form2.action = "<%=request.getContextPath() %>/secure/excel/deleteversion.wss?checkmode=${model.mode }";
	     this.form2.submit();
	}
}
...
function deleteversionmul(){
    ...
    if(! v ){return false; }
    if(v){
        form2.action = "<%=request.getContextPath() %>/secure/excel/deleteversionmul.wss";
        this.form2.submit();
    }
}
...
<TABLE WIDTH="100%" CELLPADDING="0" CELLSPACING="0" BORDER="0">
  ...
  <TD CLASS="wpsPortletArea" COLSPAN="3" >
        ...
        <h1 id="title-bread-crumb">恢复以前SNMP</h1>
        <p ></p>
...
<table border="0" cellpadding="3" cellspacing="0" valign="top" width="100%" summary="Framing Table" CLASS="button-section">
 ...
 <!-- ncc -->
		...
		<table class="framing-table" border="0" bordercolor="#999999" cellpadding="3" cellspacing="1" width="100%">
            	...
            	<legend>&nbsp;&nbsp;请选择要恢复的SNMP版本： </legend>
				<table class="framing-table" border="0" bordercolor="#999999" cellpadding="3" cellspacing="1" width="100%">
 
<%--				<tr class="table-row">
					<td class="tdlabel" width="30%">版本:<input name="versionall" type="hidden" value="${fn:length(model.bksnmp) }" id="verCount"></td>
				...
				<tr class="table-row">
					...
					<option value="${dspver.ver}"><fmt:formatDate value="${dspver.ver}" type="both"/>&nbsp;${dspver.syslog }</option>
					</c:forEach>
					</select>
					</td>
				</tr>
