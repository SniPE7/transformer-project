...
<TABLE WIDTH="100%" CELLPADDING="0" CELLSPACING="0" BORDER="0">
  ...
  <TD CLASS="wpsPortletArea" COLSPAN="3" >
    ...
    <tr> 
    	...
    	<td valign="top"><input value="新建"  name="button_new" onClick="New()" type="button" class="buttons" id="functions"></td>
							<td nowrap="nowrap">&nbsp; <input value="保存"  onClick="saveForm()" type="button" class="buttons" id="functions"> <br>
						  <input value="" name="Lev" type="hidden"></td>
    </tr>
    ...
    <input type="hidden" name="buttonperspective" value="null"/> 
     <input type="hidden" name="formAction" value="prepare"/>  
		...
		<input type="hidden" name="nmsid" value="${model.server.nmsid }">
             ...
             <td class="table-row"  nowrap>
                <table cellspacing="1" cellpadding="0" border="0" width="100%" class="framing-table">
                   <tbody>
					...
					<td><input id="nmsnameId" name="nmsname" class="input" type="text" value="${model.server.nmsname }" maxlength="64"></td>
					<td nowrap="nowrap">用户名:<font color="#ff0000"></font> </td>
					<td><input name="username" class="input" type="text" value="${model.server.username }" maxlength="64"></td>
				</tr>
				<tr class="table-row">
					<td nowrap="nowrap">服务器 IP:<font color="#ff0000">*</font> </td>
					<td><input id="nmsipId" name="nmsip" class="input" type="text" value="${model.server.nmsip }" maxlength="64"></td>
					<!-- c:if test="${model.type }=='new'"  --><td nowrap="nowrap">密码:<font color="#ff0000"></font></td><td><input name="password" class="input" type="password" value="${model.server.password }" maxlength="64"></td><!-- /c:if -->
				</tr>
				...
				<tr class="table-row">
					...
					<c:if test="${model.server.ostype == 'unix' }">
					<option value="unix" selected="selected">unix</option>
					<option value="linux">linux</option></c:if>
					<c:if test="${model.server.ostype == 'linux' }">
					<option value="unix">unix</option>
					<option value="linux" selected="selected">linux</option></c:if>
					<c:if test="${model.server.ostype != 'unix' and model.server.ostype != 'linux'}">
					<option value="unix" selected="selected">unix</option>
					<option value="linux">linux</option></c:if>
					</select></td>
					<!-- td><input name="ostype" class="input" type="text" value="${model.server.ostype }"></td -->
				</tr>
				<tr class="table-row">
					<td nowrap="nowrap" valign="top">备注：</td>
					<td colspan="3"><textarea name="description">${model.server.description }</textarea>
					</td>
				</tr>
                   </tbody>
                </table>
