...
function addsyslog(){
	form1.action = "<%= request.getContextPath()%>/secure/syslog/newsyslog.wss";
	if(checkNull()){
		this.form1.submit();
	}
}
function updatesyslog(){
	form1.action = "<%= request.getContextPath()%>/secure/syslog/updatesyslog.wss";
	if(checkNull()){
		if(document.getElementById("markOldID").value != document.getElementById("markID").value || document.getElementById("manufID").value != document.getElementById("manufactureID").value){
...
var bt1 = document.getElementById("btimelist1").value;
var bt2 = document.getElementById("btimelist2").value;
var bt3 = document.getElementById("btimelist3").value;
var bt4 = document.getElementById("btimelist4").value;
var bt5 = document.getElementById("btimelist5").value;
var bt6 = document.getElementById("btimelist6").value;
var bt7 = document.getElementById("btimelist7").value;

var et1 = document.getElementById("etimelist1").value;
var et2 = document.getElementById("etimelist2").value;
var et3 = document.getElementById("etimelist3").value;
var et4 = document.getElementById("etimelist4").value;
var et5 = document.getElementById("etimelist5").value;
var et6 = document.getElementById("etimelist6").value;
var et7 = document.getElementById("etimelist7").value;
  
         var reg = "/^[0-9]{1,20}$/";
         var bt11=new Array();
         bt11 = bt1.split(":");
         alert("bt11[0] is"+ bt11[0]+"bt11[1] is---"+bt11[1]+"10 bt11[0] is--"+parseInt(bt11[0],10)+"10 bt11[1] is"+parseInt(bt11[1],10));
         alert("bt11[0]is not number? "+(bt11[0].match(reg))+"bt11[1] is not number ?"+(bt11[1].match(reg))+"contains :? "+(bt1.indexOf(":")>0)+" parseInt0 error "+(parseInt(bt11[0],10)>24) +" parseInt1 error "+(parseInt(bt11[1],10)>60));
        /* if( (bt11[0].match(reg)||bt11[1].match(reg)&& bt1.indexOf(":")<0 )||parseInt(bt11[0],10)>24 ||parseInt(bt11[1],10)>60){
          //
          alert("请输入正确的时间格式（00：00）");
          return false;
         }*/
        if(bt1.indexOf(":")<0){
        alert("check : 请输入正确的时间格式（00：00）");
        return false;
        }else{
           if(  bt11[0].match(reg) && bt11[1].match(reg) ){
                if(parseInt(bt11[0],10)<24 && parseInt(bt11[1],10)<60){
                return true;
                }
           
           }else{ 
            alert(" check number请输入正确的时间格式（00：00）");
            return false;
           }
        }
        return true;
         ...
         var bt11=new Array();
         bt11 = bt1.split(":");
         alert("bt11[0] is"+ bt11[0]+"bt11[1] is---"+bt11[1]);
        
         var bt11=new Array();
         bt11 = bt1.split(":");
         alert("bt11[0] is"+ bt11[0]+"bt11[1] is---"+bt11[1]);*/
        /*if (!bt1.match(/^[0-2][0-9]\:[0-6][0-9]\$/)) { 
        alert ('请输入正确的时间！'); 
        return false; 
        } 
        if (!bt2.match(/^[0-2][0-9]\:[0-6][0-9]\$/)) { 
        alert ('请输入正确的时间！'); 
        return false; 
        }
         if (!bt3.match(/^[0-2][0-9]\:[0-6][0-9]\$/)) { 
        alert ('请输入正确的时间！'); 
        return false; 
        } 
        if (!bt4.match(/^[0-2][0-9]\:[0-6][0-9]\$/)) { 
        alert ('请输入正确的时间！'); 
        return false; 
        }
        if (!bt5.match(/^[0-2][0-9]\:[0-6][0-9]\$/)) { 
        alert ('请输入正确的时间！'); 
        return false; 
        }
         if (!bt6.match(/^[0-2][0-9]\:[0-6][0-9]\$/)) { 
        alert ('请输入正确的时间！'); 
        return false; 
        } 
        if (!bt7.match(/^[0-2][0-9]\:[0-6][0-9]\$/)) { 
        alert ('请输入正确的时间！'); 
        return false; 
        }
         if (!et1.match(/^[0-2][0-9]\:[0-6][0-9]\$/)) { 
        alert ('请输入正确的时间！'); 
        return false; 
        } 
        if (!et2.match(/^[0-2][0-9]\:[0-6][0-9]\$/)) { 
        alert ('请输入正确的时间！'); 
        return false; 
        }
         if (!et3.match(/^[0-2][0-9]\:[0-6][0-9]\$/)) { 
        alert ('请输入正确的时间！'); 
        return false; 
        } 
        if (!et4.match(/^[0-2][0-9]\:[0-6][0-9]\$/)) { 
        alert ('请输入正确的时间！'); 
        return false; 
        }
        if (!et5.match(/^[0-2][0-9]\:[0-6][0-9]\$/)) { 
        alert ('请输入正确的时间！'); 
        return false; 
        }
         if (!et6.match(/^[0-2][0-9]\:[0-6][0-9]\$/)) { 
        alert ('请输入正确的时间！'); 
        return false; 
        } 
        if (!et7.match(/^[0-2][0-9]\:[0-6][0-9]\$/)) { 
        alert ('请输入正确的时间！'); 
        return false; 
        }*/
       }
...
<TABLE WIDTH="100%" CELLPADDING="0" CELLSPACING="0" BORDER="0">
  ...
  <TD CLASS="wpsPortletArea" COLSPAN="3" >
        ...
        <h1 id="title-bread-crumb"> <c:choose>
           <c:when test="${model.method == 'update' }">更新Syslog Event Process</c:when>
           <c:otherwise> 新建Syslog Event Process</c:otherwise>
         </c:choose></h1>
    ...
    <tr> 
       	<c:choose >
					                <c:when test="${model.method == 'update'}" >					                
					                	<td>
    		<input type="button" name="button.update" value="保存" class="buttons" onClick="updatesyslog()"  id="functions" />
    	</td>
					               </c:when>
					               <c:otherwise >
					               		<td>
    		<input type="button" name="button.new" value="保存" class="buttons" onClick="addsyslog()" id="functions" />
    	</td>
					               </c:otherwise>
					 </c:choose>
...
<table border="0" cellpadding="3" cellspacing="0" valign="top" width="100%" summary="Framing Table" CLASS="button-section">
						...
						<table cellspacing="1" cellpadding="0" border="0" width="100%" class="framing-table">
							<tbody>
								<tr class="table-row">
									...
									<td class="tdlabel" width="15%">MARK<span style="color: red">*</span>
    	...
    	<input type="text" id="markID" name="mark" size="20" value="${model.mark }" maxlength="255"/>
									...
									<td class="tdlabel" width="15%">MANUFACTURE<span style="color: red">*</span></td>
									<td>
								...
								<c:if test="${model.method == 'update' or model.method=='new'}">
								<tr class="table-row">
									<td class="tdlabel" width="15%">监控时段1<span style="color: red">*</span></td>
									...
									<td class="tdlabel" width="15%">监控时段2<span style="color: red">*</span></td>
								...
								<tr class="table-row">
									<td class="tdlabel" width="15%">FILTERFLAG1<span style="color: red">*</span></td>
									<td class="tdcontent">
									...
									<td class="tdcontent">
									<select id="filterflag2" name="filterflag2" class="input">
										<c:if test="${model.method == 'update' or model.method=='new'}">
								...
								<tr class="table-row">
							       ...
							       <td >EVENTTYPE<span style="color: red">*</span></td>
							          <td class="tdcontent">
							            <select name="eventtype" id="eventtypeID"  class="input">
					                     <c:if test="${model.method == 'update' or model.method=='new'}">
								...
								<tr class="table-row">
									<td >SUBEVENTTYPE<span style="color: red">*</span></td>
									<td class="tdcontent"><select name="subeventtype" id="subeventtypeID" class="input" >
									<c:if test="${model.method == 'update' or model.method=='new'}">
